...
class JobsController {
  ...
  static async singleJob(req, res, next) {
    try {
  ...
  static async updateJob(req, res, next) {
    try {
      const job = await Job.findByIdAndUpdate(req.params.id, req.body, { new: true }).populate('jobType', 'jobTypeName').populate('user', 'firstName lastName');
      res.status(200).json({
        success: true,
        job,
      });
  ...
  static async showJobs(req, res, next) {
    //enable search 
